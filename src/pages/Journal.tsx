...
const Journal = () => {
  ...
  const [calendarEntries, setCalendarEntries] = useState<DailyEntry[]>([]);
  ...
  const loadEntries = async () => {
    ...
    if (!error && data) {
      ...
      // Build calendar entries with signed URLs for image display
      const calEntries: DailyEntry[] = [];
      ...
      await Promise.all(
        data.map(async (entry) => {
          ...
          if (storagePath) {
            ...
            if (signedUrl) urlMap[storagePath] = signedUrl;
          }
          // Also map all other images
          if (entry.images) {
            for (const img of entry.images) {
              if (!urlMap[img]) {
                const url = await getSignedImageUrl(img);
                if (url) urlMap[img] = url;
              }
            }
          }
          calEntries.push({
  ...
  const loadEntryForDate = async (date: Date) => {
    ...
    if (data) {
      ...
      // Resolve signed URLs for any new storage paths not yet cached
      if (data.images) {
        const newUrls = { ...signedImageUrls };
            ...
            <CardContent className="p-4 md:p-6 space-y-4">
              ...
              {(images.length > 0 || previewUrls.length > 0) && (
                <div className="flex flex-wrap gap-3 mt-4">
                  {images.map((storagePath, i) => (
                    <div key={`saved-${i}`} className="relative w-24 h-24 rounded-lg overflow-hidden group">
                      <img src={signedImageUrls[storagePath] || storagePath} alt="Saved" className="w-full h-full object-cover" />
                      <button 
