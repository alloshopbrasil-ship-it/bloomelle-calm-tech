...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  images = [],
  containerSize = typeof window !== 'undefined' && window.innerWidth < 768 ? 320 : 400,
  sphereRadius = typeof window !== 'undefined' && window.innerWidth < 768 ? 160 : 200,
  dragSensitivity = 0.5,
  momentumDecay = 0.95,
  maxRotationSpeed = 5,
  baseImageScale = typeof window !== 'undefined' && window.innerWidth < 768 ? 0.15 : 0.12,
  hoverScale = 1.2,
  perspective = typeof window !== 'undefined' && window.innerWidth < 768 ? 800 : 1000,
  autoRotate = false,
  autoRotateSpeed = 0.3,
  className = ''
}) => {
  ...
  const updateMomentum = useCallback(() => {
    ...
    setVelocity(prev => {
      ...
      if (!autoRotate && !isTouchDevice && Math.abs(newVelocity.x) < 0.01 && Math.abs(newVelocity.y) < 0.01) {
        return { x: 0, y: 0 };
      }
      return newVelocity;
    });
    ...
    setRotation(prev => {
      let newY = prev.y;
      // On touch devices, always auto-rotate; on desktop, only if autoRotate is on
      if (autoRotate || isTouchDevice) {
        newY += isTouchDevice ? 0.15 : autoRotateSpeed;
      }
      if (!isTouchDevice) {
        newY += clampRotationSpeed(velocity.y);
      }
      return {
        x: SPHERE_MATH.normalizeAngle(prev.x + (isTouchDevice ? 0 : clampRotationSpeed(velocity.x))),
        y: SPHERE_MATH.normalizeAngle(newY),
        z: prev.z
      };
    });
  }, [isDragging, momentumDecay, velocity, clampRotationSpeed, autoRotate, autoRotateSpeed, isTouchDevice]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    lastMousePos.current = { x: e.clientX, y: e.clientY };
  }, [isTouchDevice, isDragging, dragSensitivity, clampRotationSpeed]);
  ...
  }, [isMounted, isTouchDevice, handleMouseMove, handleMouseUp]);
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      <div
        ...
        onClick={() => !isTouchDevice && setSelectedImage(image)}
      >
  ...
  }, [worldPositions, baseImageSize, containerSize, hoveredIndex, isTouchDevice]);
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className={`relative select-none ${isTouchDevice ? 'pointer-events-none' : 'cursor-grab active:cursor-grabbing'} ${className}`}
        style={{
          width: containerSize,
          height: containerSize,
          perspective: `${perspective}px`,
          touchAction: isTouchDevice ? 'auto' : undefined
        }}
      ...
      {!isTouchDevice && renderSpotlightModal()}
    </>
  );
};
